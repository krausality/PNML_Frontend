...
For modeling real-world systems with timing, Generalized Stochastic Petri Nets (GSPNs)~\cite{Marsan1984} extend basic Petri nets with:
\begin{itemize}
...
PNML (Petri Net Markup Language) is an XML-based interchange format for Petri nets, standardized as ISO/IEC 15909-2~\cite{PNML2004}. It provides a common syntax for storing and exchanging Petri net models across different tools.
...
Angular~\cite{AngularDocs} is a TypeScript-based web application framework developed by Google. Key concepts used in this project include:
...
\begin{itemize}
    ...
    \item \textbf{Observables (RxJS)}~\cite{RxJS}: Reactive programming patterns for handling asynchronous events and data streams
...
Angular Material~\cite{MaterialDesign} provides pre-built UI components following Google's Material Design guidelines. The project uses Material components for:
\begin{itemize}
