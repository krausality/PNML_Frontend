/**
 * @file pnml.service.ts
 * @description This service is responsible for handling PNML (Petri Net Markup Language) files.
 * It provides functionality to parse PNML XML strings into the application's internal
 * Petri net data structures (Places, Transitions, Arcs) and to serialize the internal
 * data structures back into a PNML XML string for export.
 *
 * Modularity:
 * The `PnmlService` encapsulates all logic related to the PNML format. This separation
 * is crucial for maintainability and modularity because:
 *   - **Format-Specific Logic**: All details of the PNML standard (XML structure, element names,
 *     attribute parsing) are contained within this service. If the PNML standard evolves or if
 *     support for different PNML variants is needed, changes can be localized here.
 *   - **Decoupling from Core Logic**: The core Petri net logic (e.g., simulation, analysis,
 *     data storage via `DataService`) is independent of the PNML file format. These other
 *     services interact with `PnmlService` only when they need to import from or export to PNML.
 *   - **Reusability**: The parsing and generation logic can be reused wherever PNML interaction
 *     is required.
 *
 * Key Responsibilities:
 *   - **Parsing PNML**: Takes an XML string as input, parses it using `xml2js`, and transforms
 *     the PNML elements (places, transitions, arcs, markings, labels, graphics/positions)
 *     into the application's `Place`, `Transition`, and `Arc` objects. It also extracts actions
 *     from transition labels.
 *   - **Handling Layout Information**: Parses graphical information from PNML to set positions
 *     for places, transitions, and arc anchor points. It notes if layout data is incomplete.
 *   - **Generating PNML**: Takes the current Petri net data (from `DataService`) and constructs
 *     a valid PNML XML string. This includes generating XML for places, transitions, arcs,
 *     and their associated properties like initial markings, labels, and graphical positions.
 *   - **File Download**: Implements the `writePNML` method to trigger a browser download of the
 *     generated PNML content as a `.pnml` file.
 *   - **Dependency on DataService**: It uses `DataService` to get the current Petri net data
 *     for PNML generation and to potentially store parsed data (though the current `parse` method
 *     returns the parsed elements directly).
 *   - **Layout Service Interaction**: It can interact with layout services like `LayoutSugiyamaService`,
 *     potentially to apply a layout if the PNML file lacks complete graphical information (though the
 *     current implementation primarily focuses on parsing existing layout).
 *
 * Usage:
 *   - Typically used when a user imports a PNML file or exports the current diagram to PNML format.
 *   - The `AppComponent` or other UI-handling components would invoke methods of this service
 *     in response to user actions (e.g., file open, save button click).
 */
